...
// validatePathWithSafePaths is like validatePath but also allows paths under configured safe paths.
func validatePathWithSafePaths(path, workspace string, restrict bool, safePaths []string) (string, error) {
	if workspace == "" {
		return path, fmt.Errorf("workspace is not defined")
	}
	...
	if restrict {
		...
		if isWithinWorkspace(absPath, absWorkspace) {
			return validateSymlinkResolution(absPath, absWorkspace)
		}

		// Check if path is under any safe path
		for _, safePath := range safePaths {
			absSafePath := safePath
			if !filepath.IsAbs(safePath) {
				absSafePath, _ = filepath.Abs(safePath)
			}
			if absSafePath != "" && strings.HasPrefix(absPath, absSafePath+string(filepath.Separator)) {
				return absPath, nil
			}
			// Also check if it's exactly the safe path
			if absSafePath != "" && absPath == absSafePath {
				return absPath, nil
			}
		}
...
func validateSymlinkResolution(absPath, absWorkspace string) (string, error) {
	var resolved string
	workspaceReal := absWorkspace
	var err error
	if resolved, err = filepath.EvalSymlinks(absWorkspace); err == nil {

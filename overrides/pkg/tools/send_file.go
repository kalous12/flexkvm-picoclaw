...
func NewSendFileTool(workspace string, restrict bool, maxFileSize int, store media.MediaStore, safePaths []string) *SendFileTool {
	if maxFileSize <= 0 {
...
func (t *SendFileTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	resolved, err := validatePathWithSafePaths(path, t.workspace, t.restrict, t.safePaths)
	if err != nil {

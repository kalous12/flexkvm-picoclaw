...
func NewExecToolWithConfig(workingDir string, restrict bool, config *config.Config) (*ExecTool, error) {
	...
	// Initialize safe paths with default kernel pseudo-devices
	execSafePaths := make(map[string]bool)
	for k, v := range safePaths {
		execSafePaths[k] = v
	}

	// Add user-configured safe paths
	if config != nil {
		for _, path := range config.Tools.AllowSafePaths {
...
func (t *ExecTool) guardCommand(command, cwd string) string {
	...
	if t.restrictToWorkspace {
		...
		for _, loc := range matchIndices {
			...
			// Check if path is under any safe path directory
			isUnderSafePath := false
			for safePath := range t.safePaths {
				if strings.HasPrefix(p, safePath+string(filepath.Separator)) {
					isUnderSafePath = true
					break
				}
			}
			if isUnderSafePath {
